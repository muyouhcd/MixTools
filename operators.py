    ...
    def execute(self, context):
        ...
        def create_contact_vertex_groups(input_objects, threshold_distance):
            ...
            for obj_a in input_objects:
                ...
                # # 创建并填充自身命名的顶点组，权重设为 1
                # own_vertex_group_name = f'Bip001 {obj_a_name}'
                # own_vertex_group = (obj_a.vertex_groups.new(name=own_vertex_group_name)
                #                     if own_vertex_group_name not in obj_a.vertex_groups else
                #                     obj_a.vertex_groups[own_vertex_group_name])
                # for v in obj_a.data.vertices:
                #     own_vertex_group.add([v.index], 1.0, 'REPLACE')
