...
class SplitMeshByMaterialOperator(bpy.types.Operator):
    ...
    def separate_material_faces(self, obj, material_indices, split_material):
        """使用Blender内置功能分离指定材质的面，不清理未使用材质"""
        try:
            # 进入编辑模式
            ...
            if new_objects:
                ...
                for new_object in new_objects:
                    if new_object.type == 'MESH' and new_object != obj:
                        new_obj = new_object
                        break
            
            # 如果上面的方法没找到，使用原来的方法（从选中的物体中查找）
            if not new_obj and len(bpy.context.selected_objects) > 1:
            ...
            if new_obj:
                # 直接设置物体名称为材质名称
                new_obj.name = split_material.name
                # 同时更新mesh数据块的名称
                if new_obj.data:
                    new_obj.data.name = split_material.name
                    
                    # 找到分离材质在新物体中的索引
                    split_material_index = -1
                    for i, slot in enumerate(new_obj.material_slots):
                        if slot.material == split_material:
                            split_material_index = i
                            break
                    
                    # 如果找到了材质索引，更新所有多边形的材质索引指向该材质
                    # 不清理材质槽，保留所有材质槽
                    if split_material_index >= 0:
                        for poly in new_obj.data.polygons:
                            poly.material_index = split_material_index
    ...
    def clean_material_slots(self, obj, material):
        ...
        # 如果找到了材质，先将所有多边形的索引设置为该材质的索引
        # 这样可以确保所有多边形都指向正确的材质
        if target_material_index >= 0:
            for poly in obj.data.polygons:
                poly.material_index = target_material_index
            
            # 如果材质不在第一个槽位，需要将其移动到第一个槽位
            if target_material_index > 0:
                # 将第一个槽位设置为目标材质
    ...
    def remove_material_from_object(self, obj, material):
        ...
        if material_slot_index >= 0:
            # 检查是否还有面使用该材质（理论上不应该有，因为已经分离了）
            # 但为了安全，我们先处理这些面
            remaining_faces_with_material = []
            for poly in obj.data.polygons:
                if poly.material_index == material_slot_index:
                    remaining_faces_with_material.append(poly.index)
            ...
            # 如果有其他材质槽，将使用分离材质的面设置为第一个可用材质
            # 如果没有其他材质槽，这些面应该已经被分离了，所以这种情况不应该发生
            fallback_index = 0 if len(index_mapping) > 0 else 0
            ...
            for poly in obj.data.polygons:
                ...
                if old_index in index_mapping:
                    ...
                    # 异常情况：还有面使用要移除的材质（理论上不应该发生）
                    # 设置为第一个可用材质
                    if len(index_mapping) > 0:
                        poly.material_index = 0  # 使用第一个可用材质
                    else:
                        # 如果没有其他材质槽，保持原样（虽然这种情况不应该发生）
                        pass
            ...
            # 移除材质槽后，验证并修复材质索引（确保所有索引都在有效范围内）
            max_index = len(obj.material_slots) - 1
            for poly in obj.data.polygons:
                if poly.material_index > max_index:
...
# 材质强度调整基础类
class BaseMaterialStrengthOperator(bpy.types.Operator):
    """材质强度调整的基础操作符类"""
    bl_options = {'REGISTER', 'UNDO'}
    
    def get_material_strength_values(self, context):
        """获取当前场景中的材质强度值"""
        scene = context.scene
        return {
    ...
    def apply_material_strengths(self, context, strengths):
        """应用所有材质强度设置"""
        # 映射场景属性名称到节点输入名称
        attribute_mapping = {
            'emission_strength': 'Emission Strength',
            'roughness_strength': 'Roughness',
            'metallic_strength': 'Metallic',
            'specular_strength': 'Specular',
            'specular_tint_strength': 'Specular Tint'
        }
        
        for strength_key, value in strengths.items():
            attribute_name = attribute_mapping.get(strength_key)
            if attribute_name:
                self._apply_material_attribute(context, attribute_name, value)
        
        # 特殊处理：发光节点
        strength = strengths.get('emission_strength', 0.0)
        for obj in context.selected_objects:
            if obj.type == 'MESH' and obj.data.materials:
                for mat in obj.data.materials:
    ...
    def _apply_material_attribute(self, context, attribute_name, value):
        """通用的材质属性设置方法"""
        selected_objects = context.selected_objects
        ...
        for obj in selected_objects:
            ...
            for mat in obj.data.materials:
                ...
                self._set_node_attribute(mat.node_tree.nodes, attribute_name, value)
    
    def _set_node_attribute(self, nodes, attribute_name, value):
        """设置材质节点树中的特定属性"""
        for node in nodes:
            if node.type == 'BSDF_PRINCIPLED':
                if attribute_name in node.inputs:
                    try:
                        node.inputs[attribute_name].default_value = value
...
class ApplyAllMaterialStrengths(BaseMaterialStrengthOperator):
    ...
    def execute(self, context):
        try:
            ...
            self.apply_material_strengths(context, strengths)
            
            self.report({'INFO'}, f"已应用所有材质强度设置：发光={strengths['emission_strength']:.2f}, 粗糙度={strengths['roughness_strength']:.2f}, 金属度={strengths['metallic_strength']:.2f}, 高光={strengths['specular_strength']:.2f}, 光泽度={strengths['specular_tint_strength']:.2f}")
            return {'FINISHED'}
