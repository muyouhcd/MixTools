...
class MIAO_OT_RoleReplacer(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if source_collection in bpy.data.collections:
            collection_1 = bpy.data.collections[source_collection]
            for obj in collection_1.objects:
            ...
            self.report({'ERROR'}, f"源集合 '{source_collection}' 不存在，请检查集合名称是否正确。")
            return {'CANCELLED'}

        # 遍历目标集合中的物体
        if target_collection in bpy.data.collections:
            collection_2 = bpy.data.collections[target_collection]
            
            for obj in collection_2.objects:
                if obj.type == 'MESH':  # 仅针对 Mesh 类型物体
                    for category, keywords in categories.items():
                        if any(keyword in obj.name for keyword in keywords):
                            # 检查源集合中是否有对应分类的物体
            ...
            self.report({'ERROR'}, f"目标集合 '{target_collection}' 不存在，请检查集合名称是否正确。")
            return {'CANCELLED'}
